...
# logic: Weight_New += Weight_Current * Learning_Rate * (Update_Direction)

        # GR -> GO (Excitatory)
        self.grgoW += self.grgoW * self.grgo_plast * (((self.act[t] * 0.0001) + ((self.act[t] - 1) * 0.0001 * self.plast_ratio)))

        # MF -> GO (Excitatory) - "This is working!"
        self.mfgoW += self.mfgoW * self.mfgo_plast * (((self.act[t] * 0.0001) + ((self.act[t] - 1) * 0.0001 * self.plast_ratio)))

        # GO -> GO (Inhibitory)
        # Note: Ensure gogoW values are positive if you are treating them as magnitudes, 
        # otherwise the sign logic works the same.
        self.gogoW += self.gogoW * self.gogo_plast * (((1-self.act[t]) * 0.0001) + ((self.act[t]) * -1.0 * 0.0001 * self.plast_ratio))
                # # capping
