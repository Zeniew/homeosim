...
class Golgi(): # class of Golgi cells, entire network of Golgi cells
    def __init__(self, n, csON, csOFF, useCS, numBins, gogo_weight = 0.0125, mfgo_weight = 0.0042, grgo_weight = 0.0007, plast_ratio = 1):
        ...
        self.target_spikes = 50 # target spikes over trial
    ...
    def update_thresh(self, t, threshRest):
        ...
        # This only handles negative resting threshold, but I think that's fine as resting threshold shouldn't go to positive values where it would cause spiking without any input, and if it does, the clipping at the end will fix it
        ...
        threshRest = threshRest * learning_rates

        # Clipping threshold to prevent it from going positive, but I didn't put 0 as minimum to allow for changes still
        threshRest = np.minimum(threshRest, -0.0001)
...
class Granule():
    def __init__(self, n, csOFF, csON, useCS, trialSize, mfgr_weight = 0.032, gogr_weight = 0.015):
        ### Constants
        self.numGranule = n
    ...
    def update_thresh(self, t, threshRest):
        ...
        count_float = count.astype(float)
        # 2. Calculate Error
        error = self.target_spikes - count_float # if error positive, target spikes > actual spikes, if error negative, target spikes < actual spikes
