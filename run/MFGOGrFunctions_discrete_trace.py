...
class Golgi(): # class of Golgi cells, entire network of Golgi cells
    ...
    def do_Golgi(self, t):
        ...
        if self.mfgo_plast == 1:  # update MF to Golgi weight
            self.update_weight(self.mfgoW, self.MFGO_trace, 1, t, self.mfgo_LTP_inc, self.mfgo_LTD_inc) # update MF to Golgi weight
        if self.grgo_plast == 1:  # update granule to Golgi weight
            self.update_weight(self.grgoW, self.GRGO_trace, 1, t, self.grgo_LTP_inc, self.grgo_LTD_inc) # update granule to Golgi weight
        if self.gogo_plast == 1:  # update Golgi to Golgi weight
            self.update_weight(self.gogoW, self.GOGO_trace, 2, t, self.gogo_LTP_inc, self.gogo_LTD_inc) # update Golgi to Golgi weight
        ## Discrete
    ...
    def update_weight(self, weight_array, trace_array, exc_or_inh, t, LTP, LTD):
        ...
        learning_rates = np.where(error > 0, LTP, LTD) # use alpha_up if error is positive, alpha_down otherwise

        # 5. Additive Plasticity
        weight_array += learning_rates
